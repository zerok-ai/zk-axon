...
const (
	...
	GetTraceQueryByScenarioId                                    = "SELECT CASE WHEN $1 THEN COUNT(*) OVER() ELSE 0 END AS total_rows, trace_id, incident_collection_time, service_name, span_name, protocol, start_time, latency FROM (SELECT DISTINCT ON (incident.trace_id) incident.trace_id, incident.incident_collection_time, span.service_name, span.span_name, span.protocol, span.start_time, span.latency FROM issue INNER JOIN incident ON issue.issue_hash = incident.issue_hash INNER JOIN span ON incident.trace_id = span.trace_id WHERE issue.scenario_id = $2 AND span.is_root=$3 AND issue.issue_hash=$4) AS distinct_incidents ORDER BY incident_collection_time DESC LIMIT $5 OFFSET $6"
	GetTraceQueryByScenarioIdWithoutIssueHashFilter              = "SELECT CASE WHEN $1 THEN COUNT(*) OVER() ELSE 0 END AS total_rows, trace_id, incident_collection_time, service_name, span_name, protocol, start_time, latency FROM (SELECT DISTINCT ON (incident.trace_id) incident.trace_id, incident.incident_collection_time, span.service_name, span.span_name, span.protocol, span.start_time, span.latency FROM issue INNER JOIN incident ON issue.issue_hash = incident.issue_hash INNER JOIN span ON incident.trace_id = span.trace_id WHERE issue.scenario_id = $2 AND span.is_root=$3) AS distinct_incidents ORDER BY incident_collection_time DESC LIMIT $4 OFFSET $5"
)
...
func (z tracePersistenceRepo) GetSpans(traceId, spanId string, offset, limit int) ([]dto.SpanTableDto, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(&rawData.TraceID, &rawData.ParentSpanID, &rawData.SpanID, &rawData.SpanName, &rawData.IsRoot,
			&rawData.Kind, &rawData.StartTime, &rawData.Latency, &rawData.Source, &rawData.Destination,
			&rawData.WorkloadIDList, &rawData.Protocol, &rawData.IssueHashList, &rawData.RequestPayloadSize,
			&rawData.ResponsePayloadSize, &rawData.Method, &rawData.Route, &rawData.Scheme, &rawData.Path,
			&rawData.Query, &rawData.Status, &rawData.Metadata, &rawData.Username, &rawData.SourceIP,
			&rawData.DestinationIP, &rawData.ServiceName, &rawData.Errors)
