...
const (
	...
	GetTraceQueryByScenarioId         = "SELECT CASE WHEN $1 then COUNT(*) OVER() ELSE 0 END AS total_rows, DISTINCT incident.trace_id, incident.incident_collection_time FROM issue INNER JOIN incident ON issue.issue_hash = incident.issue_hash WHERE issue.scenario_id = $2 ORDER BY incident_collection_time DESC LIMIT $3 OFFSET $4"
