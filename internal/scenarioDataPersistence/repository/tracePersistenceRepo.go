...
const (
	...
	//TODO: Make changes here to work with new db schema.
	GetTraceQueryByScenarioId         = "SELECT CASE WHEN $1 THEN COUNT(*) OVER() ELSE 0 END AS total_rows, trace_id, incident_collection_time FROM (SELECT DISTINCT ON (incident.trace_id) incident.trace_id, incident.incident_collection_time FROM issue INNER JOIN incident ON issue.issue_hash = incident.issue_hash WHERE issue.scenario_id = $2) AS distinct_incidents ORDER BY incident_collection_time DESC LIMIT $3 OFFSET $4;"
...
func (z tracePersistenceRepo) GetTracesForScenarioId(scenarioId string, offset, limit int) ([]dto.IncidentTableDto, error) {
	...
	for rows.Next() {
		...
		//TODO: Make changes here to work with new db schema.
		err := rows.Scan(&rawData.TotalRows, &rawData.TraceId, &rawData.IncidentCollectionTime)
		if err != nil {
			zkLogger.Error(LogTag, logMessage, err)
		}
